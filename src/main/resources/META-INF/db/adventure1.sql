...
update order_detail set review = 'Y' WHERE ODID=4;
COMMIT;
...
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT * FROM REVIEW ORDER BY RRDATE DESC) A) WHERE RN BETWEEN 1 AND 10;

INSERT INTO REVIEW (RID, RTITLE, RCONTENT, RSCORE, RPHOTO, MID, ODID)
VALUES (REVIEW_SEQ.NEXTVAL, '즐거웠습니다', '최고에요', 5, 'noimg.jpg', 'trio', 37);

INSERT INTO REVIEW (RID, RTITLE, RCONTENT, RSCORE, RPHOTO, MID, ODID)
VALUES (REVIEW_SEQ.NEXTVAL, '에효', 'ㄹㅇㅇㄱ', 1, 'noimg.jpg', 'trio', 4);
select * from review;


SELECT 
  (SELECT COUNT(*) FROM REVIEW) REVIEWCOUNT,
  (SELECT NVL(AVG(rscore),2) FROM REVIEW) AVG
FROM DUAL;


SELECT ROWNUM RN, A.RID, A.RTITLE, A.RCONTENT, A.RSCORE, A.RPHOTO, A.RRDATE, A.MID, A.OTYPE, A.OATNAME1, R.ODID, A.OATNAME2, A.OATNAME3, A.OVISITDATE FROM (SELECT R.*, OD.* FROM REVIEW R, ORDER_DETAIL OD WHERE R.ODID = OD.ODID ORDER BY R.RRDATE) A;

SELECT ROWNUM RN, A.RID, A.RTITLE, A.RCONTENT, A.RSCORE, A.RPHOTO, A.RRDATE, A.MID, A.OTYPE, A.OATNAME1, A.ODID, A.OATNAME2, A.OATNAME3, A.OVISITDATE
FROM (
    SELECT R.*, OD.*
    FROM REVIEW R
    JOIN ORDER_DETAIL OD ON R.ODID = OD.ODID
    ORDER BY R.RRDATE
) A;

SELECT * FROM 
(SELECT ROWNUM RN, A.RID, A.RTITLE, A.RCONTENT, A.RSCORE, A.RPHOTO, A.RRDATE, A.MID, A.OTYPE, A.OATNAME1, A.R_ODID, A.MNAME, A.OATNAME2, A.OATNAME3, A.OVISITDATE
FROM (
    SELECT R.*, OD.ODID AS R_ODID, OD.*, M.MNAME
    FROM REVIEW R, ORDER_DETAIL OD, MEMBER M WHERE R.ODID = OD.ODID AND R.MID = M.MID
    ORDER BY R.RRDATE DESC) A) 
    WHERE RN BETWEEN 1 AND 5;
    
    SELECT R.*, OD.ODID AS R_ODID, OD.*, M.MNAME
    FROM REVIEW R, ORDER_DETAIL OD, MEMBER M WHERE R.ODID = OD.ODID AND R.MID = M.MID and RID = 9;
...
SELECT O.*, OD.ODID, OD.OP1, OD.OP2, OD.OTYPE, 
                ...
                WHERE O.OID = OD.OID AND MID = 'trio' ORDER BY O.OID DESC;

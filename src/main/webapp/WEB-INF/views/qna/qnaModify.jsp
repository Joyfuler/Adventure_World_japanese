...
<c:if test="${empty member }"> 
	<script>
	   alert('로그인 후 문의글 수정이 가능합니다');
	   location.href = '${conPath}/member/loginMember.do?next=qna/qnaModify.do?qno=${param.qno}';
	</script>
</c:if>
	<div class="qna">
	      <div class="qnaimg" style="background-image:url('${conPath}/images/qnaaa.png');"></div>
	  	  <div class="qnaBox">
		      ...
		      <form action="${conPath }/qna/qnaModifyConfirm.do" class="fix02" name="qna" method="post">
		         <input type="hidden" name="command" value="qnaWrite">
		         <input type="hidden" name="mid" value="${member.mid }">
		         <input type = "hidden" name = "qno" value = "${originalQnaInfo.qno }">		         
		          <table class="fix03">
		             <tr>
		                <th>Secret mode</th>
		                <td>
		                   <input type="checkbox" name="check" value="secret" onchange="enabled1()"
		                   <c:if test = "${originalQnaInfo.qpwchk eq 'Y' }">
		                   checked = "checked"
		                   </c:if>
		                   > &nbsp;비밀글로 하기&nbsp;&nbsp;
		                   <input type="password" name="qpw" id="pass" size="15" style = "border : 1px solid gray;"	                     
		                    <c:if test = "${empty originalQnaInfo.qpwchk or originalQnaInfo.qpwchk eq 'N' }">		                    
		                    disabled="disabled" 
		                    </c:if>
		                    >
		               </td>
		            ...
		            <tr>
		               ...
		               <td width="500" style="text-align:left;">
		                  <input type="text" name="qtitle" size="50" autofocus="autofocus" value = "${originalQnaInfo.qtitle }" >
		               </td>
		            </tr>
		            <tr>
		               <th>내용</th>
		               <td align="left" style="text-align:left;font-size:115%;">		               
		               <textarea id="summernote" name="qcontent">${originalQnaInfo.qcontent } </textarea>
		               </td>
		            </tr>
		          </table>
				<br>
			      <div class="buttons">
			         <input type="submit" value="글수정" class="purpleBtn" style="border:none;" > 

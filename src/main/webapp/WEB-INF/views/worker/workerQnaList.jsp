...
<jsp:include page="workerHeader.jsp"/>
	<form action="${conPath }/worker/workerQnaList.do" name="frm" method="post">
	...
	<div id ="paging" s style="margin-top: -50px;">
			<c:if test="${paging.startPage > paging.blockSize}">
				[ <a href="${conPath }/worker/workerQnaList.do?pageNum=${paging.startPage-1 }&schWord=${param.schWord}">이전</a> ]
			</c:if>	
			<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage }">
				...
				<c:if test="${paging.currentPage != i }">
					[ <a href="${conPath }/worker/workerQnaList.do?pageNum=${i }&schWord=${param.schWord}">${i }</a> ]
				</c:if>
			</c:forEach>
			<c:if test="${paging.endPage<paging.pageCnt }">
				[ <a href="${conPath }/worker/workerQnaList.do?pageNum=${paging.endPage+1 }&schWord=${param.schWord}">다음</a> ]
			</c:if>
		</div>
